...
./configure --enable-cli=no --host ${TARGET} --prefix="${PWD}/../distribution" --exec-prefix="${PWD}/../distribution/${ABI}" CFLAGS='-fPIC -Din_addr_t=uint32_t' CXXFLAGS='-fPIC -Din_addr_t=uint32_t'
make
make install
